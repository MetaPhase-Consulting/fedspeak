...
FedSpeak is not itself a federal information system — it's a public reference
tool MetaPhase builds and operates, with no ATO in flight and none pursued
(see [`challenge-ato.md`](.challengeai/challenge-ato.md)). Several tool files
below say so directly rather than inventing federal posture this repository
...
what applies to every federal system regardless of stack (and where FedSpeak
specifically opts out, as a non-federal system). Then read the tool files for
